 ...
 * @param key 环境变量名
 */
function envCommaArray(key: string, isNumber: boolean = false): Array<any> {
...
export module TaskConfig {
  ...
  const biliApiDelay = envCommaArray('BILI_API_DELAY', true);
  /** 调用api时的延迟(单位s),默认2s至6s */
  export const BILI_API_DELAY: number[] = biliApiDelay.length
    ? biliApiDelay
    : [2, 6];
